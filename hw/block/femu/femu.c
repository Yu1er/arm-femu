...
static void nvme_process_cq_cpl(void *arg)
{
    ...
    while ((req = pqueue_peek(n->pq))) {
        ...
        if (now - req->expire_time >= 20000) {
            n->nr_tt_late_ios++;
            if (n->print_log) {
                printf("Coperd,%s,diff,pq.count=%lu,%" PRId64 ", %lu/%lu\n",
    ...
    NvmeRequest *req)
{
    if(cmd->opcode == NVME_CMD_REMOTE_READ) {   //NVME_CMD_REMOTE_READ and remote_parity_read
        ...
        if(req->is_remote_slba) {
            ...
            req->remote_entry_offset = rw->rsvd12[1];
        }
        else {
            req->src_slba = rw->rsvd2[0];
        }
...
// uint16_t nvme_trim(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
//     NvmeRequest *req)
// {
//     NvmeDsmCmd *rw = (NvmeDsmCmd *)cmd;
...
//         req->slba = rw->rsvd2[0];
//     }
//     else if(rw->rsvd12[0] == 1) {
//         req->slba = rw->rsvd2[0];
//         req->remote_entry_offset = rw->rsvd12[1];
//     }
//     else if (rw->rsvd12[0] == -1) {
//         req->is_special_cmd = true;
//         req->special_value = rw->rsvd12[1];
//         if(req->special_value < -2) {
//             printf("TRIM error in FEMU_memory, rw->rsvd12[0]=%lu\n", rw->rsvd12[1]);
//             return NVME_DNR;
//         }
//         return NVME_SUCCESS;
//     }
...
//     if (req->slba + req->nlb > le64_to_cpu(ns->id_ns.nsze)) {
//         // nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_LBA_RANGE,
//         //         offsetof(NvmeCmd, cdw10), req->slba + req->nlb, ns->id);
//         printf("TRIM error in FEMU_memory, is_remote=%d, slba=%lu, entry_offset=%lu\n", 
//                                         rw->rsvd12[0], rw->rsvd2[0], rw->rsvd12[1]);
...
void nvme_process_sq_io(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq))) {
        ...
        //should update req->slba before nvme_io_cmd, otherwise read/write data from/into an error slba
        ...
        if (status == NVME_SUCCESS) {
            ...
            printf("FEMU: Error IO processed! op=%d, status=%d\n", req->opcode, status);
        }
...
static int femu_init(PCIDevice *pci_dev)
{
    ...
    //femu_init_mem_backend(&n->remote_mbe, bs_size);    //wen:added
...
static void femu_exit(PCIDevice *pci_dev)
{
    ...
    //femu_destroy_mem_backend(&n->remote_mbe);
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_UINT32("ch_xfer_lat", FemuCtrl, ch_xfer_lat, 60000),
